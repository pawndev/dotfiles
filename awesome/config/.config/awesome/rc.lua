...
kbdcfg = layout_indicator({
    layouts = {
        ...
        {name="dv",  layout="us",  variant="dvorak-intl"},
    }
})
...
local function set_wallpaper(s)
    if beautiful.wallpaper then
        ...
        awful.spawn.with_shell("feh --recursive --randomize --bg-fill ".. beautiful.wallpaper)
    end
end
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey }, "\\", function() awful.spawn.with_shell("feh --recursive --randomize --bg-fill ~/Pictures/Wallpapers") end,
    ...
    awful.key({ "Mod4", "Mod1"    }, "space", function () naughty.destroy_all_notifications() end,
              {description = "dismiss notifications boulou", group = "awesome"}),
    awful.key({  }, "Print", function ()
        awful.util.spawn_with_shell("sh ~/bin/take-screenshot whole ~/Pictures/Screenshots")
        naughty.notify({ text = "Screenshot taken", icon = icons.camera })
    end,
            {description = "take screenshot whole screen", group = "launcher"}),
    awful.key({ "Shift" }, "Print", function ()
        awful.util.spawn_with_shell("sleep 0.5 && sh ~/bin/take-screenshot region ~/Pictures/Screenshots")
        naughty.notify({ text = "Screenshot selection", icon = icons.camera })
    end,
            {description = "take screenshot region", group = "launcher"}),
    awful.key({ "Control" }, "Print", function ()
        awful.util.spawn_with_shell("sh ~/bin/take-screenshot window ~/Pictures/Screenshots")
        naughty.notify({ text = "Screenshot taken", icon = icons.camera })
    end,
    ...
    awful.key({ modkey, "Shift"   }, "r", awesome.restart,
              {description = "reload awesome", group = "awesome"}),
    -- Exit screen
    awful.key({ modkey, "Shift"   }, "x", function() awful.spawn.with_shell("sh ~/bin/i3exit lock") end,
...
awful.rules.rules = {
    ...
    -- Disable titlebars to normal clients
