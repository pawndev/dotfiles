...
local helpers = require("noodle.helpers")
...
local user_picture = wibox.widget {
    wibox.widget.imagebox(icons.pikachu),
...
local bookmarks = wibox.widget {
    ...
    pad(1),
    create_bookmark("SCREENSHOTS", "~/Pictures/Screenshots"),
    layout = wibox.layout.fixed.vertical
}
...
local urls = wibox.widget {
    create_url("TRENDING", "github.com/trending"),
    pad(1),
    create_url("REDDIT", "reddit.com/r/unixporn"),
    pad(1),
    create_url("GITHUB", "github.com/pawndev"),
    layout = wibox.layout.fixed.vertical
}
...
local brightness_icon = wibox.widget.imagebox(icons.redshift_icon)
...
local notification_state = wibox.widget.imagebox(icons.alarm_icon)
...
local function update_notification_state_icon()
    if naughty.is_suspended() then
        notification_state.image = icons.alarm_off_icon
    else
        notification_state.image = icons.alarm_icon
    end
end
...
local screenshot = wibox.widget.imagebox(icons.screenshot_icon)
...
screenshot_box:buttons(gears.table.join(
                 ...
                 awful.button({ }, 1, function ()
                     awful.spawn.with_shell("sh ~/bin/take-screenshot whole ~/Pictures/Screenshots")
                 end),
                 -- Right click - Take screenshot in 5 seconds
                 awful.button({ }, 3, function ()
                     naughty.notify({title = "Say cheese!", text = "Taking shot in 5 seconds", timeout = 4, icon = icons.screenshot_icon})
                     awful.spawn.with_shell("sleep 5 && sh ~/bin/take-screenshot whole ~/Pictures/Screenshots")
                 end)
))
