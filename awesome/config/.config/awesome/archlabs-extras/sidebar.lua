...
local archlabs_icon = wibox.widget.imagebox(icons.pikachu)
...
if beautiful.sidebar_hide_on_mouse_leave then
  local sidebar_activator_top = wibox({y = sidebar.y, width = 1, visible = true, ontop = true, opacity = 0, below = true})
  local sidebar_activator_bottom = wibox({y = awful.screen.focused().geometry.height - dpi(5), width = 1, visible = true, ontop = true, opacity = 0, below = true})
  sidebar_activator_top.height = 10
  sidebar_activator_bottom.height = 10
  -- sidebar_activator.height = sidebar.height - beautiful.wibar_height
  sidebar_activator_top:connect_signal("mouse::enter", function ()
                                     sidebar.visible = true
  end)

  sidebar_activator_bottom:connect_signal("mouse::enter", function ()
    sidebar.visible = true
  end)

  if beautiful.sidebar_position == "right" then
    sidebar_activator_top.x = awful.screen.focused().geometry.width - sidebar_activator_top.width
    sidebar_activator_bottom.x = awful.screen.focused().geometry.width - sidebar_activator_bottom.width
  else
    sidebar_activator_top.x = 0
  end

  sidebar_activator_top:buttons(
    gears.table.join(
      -- awful.button({ }, 2, function ()
      --     start_screen_show()
